...
"""
    ...
    The second letter refers to the location of the metabolite
        c = cytosol
        m = mitochondria
        e = extracellular
    The following characters are identifiers for the metabolite
    See supplementary material of O'Brien paper (pg22)
"""
Cc23p2g = 3.1   # 23P2Gc
#Ccatp = 0.311  # ATPc
Ccamp = 0.03    # AMPc
...
CcNa = 10       # intracellular sodium concentration in mM - usually cited as 5-15mM
Ccnh3 = 1.5 #cytosolic ammonia concentration # for gs reaction

### Enzyme activity scaling
'''
    Unsure what this refers to, Yen-An said he doesn't touch it.
    Also in supplementary of O'Brien paper (pg26)
'''
Kx12 = 0.15
Kx14 = 0.17
Kx15 = 0.197
...
'''
    ...
    Not in supplementary material
'''
fct = 1
fct2 = 9.9989 * 2.25

### These look like volumes for concentration calculations?
# Volume of cytosol
Vc = (0.75) * 1.5 * 10 ** -9  # ml
...
'''
    Probably defines parameters for outside the cell? Some are marked with Cc* though.
'''
#CcH = 1 * 10 ** (-7.0)  # molar
...
'''
   This next section seems to be listing the parameters for the equations in the supplementary material. 
'''
## Glycolysis
...
'''
    Sometimes there are additional isoforms for each enzyme
'''
# Hexokinase
# HK1
Khkmgatp = 1.0
...
Kpfklf6p = 1.36 #new 1.36, https://doi.org/10.1042/BCJ20200656 #old 0.09
Kpfklmgatp = 0.068
Kpfklatp = 0.16# new 0.16, https://doi.org/10.1042/BCJ20200656 #old 0.1
Kpfklamp = 0.3
Kpfkmgl = 0.2
Lpfk = 0.002
...
Kpfkmafbp = 0.35  # [1]
Kpfkmg16p = Kpfklg16p
Kpfkmf6p = 0.147 #new 0.147, https://doi.org/10.1042/BCJ20200656 #old Kpfklf6p
Kpfkm23p2g = Kpfkl23p2g
Kpfkmatp = 0.152 #new 0.152, https://doi.org/10.1042/BCJ20200656 #old Kpfklatp
Kpfkmamp = Kpfklamp
...
Kpfkpg16p = Kpfklg16p
Kpfkpf6p = 1.333 #new 1.333, https://doi.org/10.1042/BCJ20200656 #old Kpfklf6p  # 0.15
Kpfkp23p2g = Kpfkl23p2g
Kpfkpatp = 0.276 #new 0.276, https://doi.org/10.1042/BCJ20200656 #old Kpfklatp  # 0.7
Kpfkpamp = Kpfklamp
...
# Glutamine synthetase, parameters from HEPATOKIN1
Kgsglu = 5
Kgsnh3 = 0.3
Kgsatp = 1.2
...
# glutamine-sodium transporter (extracellular->cytosol), DOI: 10.3389/fchem.2014.00061
Kexglnna = 0.039
Kcnglnna = 1.8
# Kglnna = 4.5 # old
Vglnna = 345 # guessed Vmax,0 depends on targetting intracellular gln conc.
...
# new asparagine-sodium transporter (extracellular->cytosol), DOI: 10.1007/s00726-014-1808-x
Kexasnna = 0.067
Kcnasnna = 1.6
Vaspna = 0.95 # guessed Vmax,0 depends on targetting intracellular asn conc.

# Vasnna = 225
# Kexaspna = 16

# new asparaginase, doi: 10.1126/science.1160809
Kmaspgasn = 2.09
Vaspg = 25 # guessed Vmax,0 depends on targetting intracellular asp conc.

'''
    These look like additonal parameters for gluconeogenesis, not from O'Brien paper but from Yen-An's paper.
'''
...
def set_KP(param: str, value: float) -> float:
	"""
	Sets a value of for a kinetic parameter
	Args:
		param (string): Name of kinetic parameter
		value (float): New value of kinetic parameter
    Returns:
        new_KP (float): Changed value of kinetic parameter
	"""
	for x in globals():
		if x==param:
			globals()[x]=value; return value;
   
def perturb_KP(param: str,pert: float) -> float:
    """
	Perturbs a kinetic parameter parameter for sensitivity analysis
	Args:
		param (string): Name of kinetic parameter to be perturbed 
		pert (float): Perturbation value
    Returns:
        perturbed_KP (float): Perturbed kinetic parameter value
	"""
    for x in globals():
        if x==param:
            return globals()[x]*(1+pert)